    ...
    return struct {
        ...
        pub noinline fn onFileUpdate(
            ...
            for (events) |event| {
                ...
                switch (kind) {
                    .file => {
                        if (event.op.delete or (event.op.rename and Environment.isDarwin)) {
                            ctx.removeAtIndex(
                                event.index,
                                0,
                                &.{},
                    ...
                    .directory => {
                        ...
                        if (affected.len > 0 and !Environment.isDarwin) {
                            if (rfs.entries.get(file_path)) |existing| {
                                this.putTombstone(file_path, existing);
                                entries_option = existing;
                            } else if (this.getTombstone(file_path)) |existing| {
                        ...
                        if (entries_option) |dir_ent| {
                            ...
                            for (affected) |changed_name_| {
                                const changed_name: []const u8 = if (comptime Environment.isDarwin)
                                    changed_name_
                                else
